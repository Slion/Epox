...
CHashIncrementalHashWithReplicateStep::~CHashIncrementalHashWithReplicateStep()
	{
	}
...
CHashIncrementalHashWithReplicateStep::CHashIncrementalHashWithReplicateStep()
	{
	SetTestStepName(KHashIncrementalHashWithReplicateStep);
	}


TVerdict CHashIncrementalHashWithReplicateStep::doTestStepPreambleL()
	{
...
TVerdict CHashIncrementalHashWithReplicateStep::doTestStepL()
	{
	if (TestStepResult()==EPass)
		...
		INFO_PRINTF1(_L("*** Hash - Incremental Hash with Replicate ***"));
				...
				if(err != KErrNone)
	    			...
	    			if(err != KErrNone)
						...
						if(*hashResult == expectedHash)
							{
							INFO_PRINTF1(_L("*** Hash - Incremental Hash with Replicate : PASS ***"));
							SetTestStepResult(EPass);	
							}
						else
							{
...
TVerdict CHashIncrementalHashWithReplicateStep::doTestStepPostambleL()
	{
