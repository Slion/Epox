...
CHashIncrementalWithResetStep::~CHashIncrementalWithResetStep()
	{
	}
...
CHashIncrementalWithResetStep::CHashIncrementalWithResetStep()
	{
	SetTestStepName(KHashIncrementalWithResetStep);
	}


TVerdict CHashIncrementalWithResetStep::doTestStepPreambleL()
	{
	return EPass;
	}


TVerdict CHashIncrementalWithResetStep::doTestStepL()
	{
	//Assume faliure, unless all is successful
	SetTestStepResult(EFail);

	INFO_PRINTF1(_L("*** Hash - Incremental with Reset ***"));
			...
			if(err != KErrNone)
    			...
    			if(err != KErrNone)
					...
					if(*hashResult == expectedHash)
						{
						INFO_PRINTF1(_L("*** Hash - Incremental with Reset : PASS ***"));
...
TVerdict CHashIncrementalWithResetStep::doTestStepPostambleL()
	{
