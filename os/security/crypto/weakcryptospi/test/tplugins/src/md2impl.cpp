...
CMD2Impl* CMD2Impl::NewL(TUid aImplementationUid)
	{
	CMD2Impl* self=new (ELeave) CMD2Impl(aImplementationUid);
	return self;						
	}
														
CMD2Impl* CMD2Impl::NewLC(TUid aImplementationUid)
	{
	CMD2Impl* self=NewL(aImplementationUid);
	CleanupStack::PushL(self);
	return self;						
	}
														
CMD2Impl::CMD2Impl(TUid aImplementationUid) : iImplementationUid(aImplementationUid), iHash(KMD2HashSize)
	{		
	}
...
MHash* CMD2Impl::ReplicateL()
	{
	return NewL(iImplementationUid);	
	}
