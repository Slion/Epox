...
struct test_start_caller {
   	EXPORT_C test_start_caller( test_observer* to, counter_t tc_amount )
    : m_to( to )
    , m_tc_amount( tc_amount )
    {}

   EXPORT_C int operator()()
    {
...
public:
  EXPORT_C   framework_impl()
   ...
   EXPORT_C   void            set_tu_id( test_unit& tu, test_unit_id id ) { tu.p_id.value = id; }

    // test_tree_visitor interface implementation
  EXPORT_C    void            visit( test_case const& tc )
    {
   ...
   EXPORT_C   void            test_suite_finish( test_suite const& ts )
    {
        BOOST_TEST_FOREACH( test_observer*, to, m_observers )
