...
Ctransient_utcStep::~Ctransient_utcStep()
/**
 * Destructor
 */
	{
 ...
 */
	...
	SetTestStepName(Ktransient_utcStep);
	}

TVerdict Ctransient_utcStep::doTestStepPreambleL()
/**
 ...
 */
	...
	User::LeaveIfError(RProperty::Define(KTestUid, KTestKey5, 0));
...
@SYMPREQ					PREQ234
*/
TVerdict Ctransient_utcStep::doTestStepL()
/**
 * @return - TVerdict code
 ...
 */
	...
	_LIT(KTestName1, "Transient UTC - Time-Based");
	_LIT(KTaskData1, "This is some really exciting task data (number 1)");
	_LIT(KTestName2, "Transient UTC - Condition-Based");
	...
	TTsTime startTimeForSchedule(datetime1, ETrue); // 10:00 am
	...
	TESTL(SchSvrHelpers::IsTimeTheSameNoSeconds(TTsTime(timeNow, ETrue), startTimeForSchedule));

	STaskSemaphore::WaitL();	// Now wait for the condition-based schedule to fire

	timeNow.UniversalTime();
	TESTL(SchSvrHelpers::IsTimeTheSameNoSeconds(TTsTime(timeNow, ETrue), defaultRuntime));
...
TVerdict Ctransient_utcStep::doTestStepPostambleL()
/**
 * @return - TVerdict code
 * Override of base class virtual
 */
