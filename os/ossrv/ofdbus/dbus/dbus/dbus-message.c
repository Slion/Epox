...
#if EMULATOR
GET_GLOBAL_VAR_FROM_TLS(_dbus_lock_message_cache,dbus_message,DBusMutex *)
...
#else
GET_STATIC_VAR_FROM_TLS(message_cache_count,dbus_message,int)
#define message_cache_count (*GET_DBUS_WSD_VAR_NAME(message_cache_count,dbus_message,s)())

GET_STATIC_VAR_FROM_TLS(message_cache_shutdown_registered,dbus_message,dbus_bool_t)
#define message_cache_shutdown_registered (*GET_DBUS_WSD_VAR_NAME(message_cache_shutdown_registered,dbus_message,s)())

GET_STATIC_ARRAY_FROM_TLS(message_cache_array,dbus_message,DBusMessage *)
#define message_cache_array (GET_DBUS_WSD_VAR_NAME(message_cache_array,dbus_message,s)())
...
#else      
      if (message_cache_array[i])
        dbus_message_finalize (message_cache_array[i]);
#endif
  ...
  while (i < MAX_MESSAGE_CACHE_SIZE)
    {
      ...
      if (message_cache_array[i])
        {
          message = message_cache_array[i];
          message_cache_array[i] = NULL;
          message_cache_count -= 1;
          break;
        }
...
#else
  while (message_cache_array[i] != NULL)
#endif  
    ++i;
...
#else
  _dbus_assert (message_cache_array[i] == NULL);
  message_cache_array[i] = message;
#endif  
  message_cache_count += 1;
  was_cached = TRUE;
...
#if EMULATOR
GET_STATIC_VAR_FROM_TLS(slot_allocator,dbus_message,DBusDataSlotAllocator)
#define slot_allocator (*GET_DBUS_WSD_VAR_NAME(slot_allocator,dbus_message,s)())
#else
static DBusDataSlotAllocator slot_allocator;
#endif

#if EMULATOR
GET_GLOBAL_VAR_FROM_TLS(_dbus_lock_message_slots,dbus_message,DBusMutex *)
