...
static dbus_bool_t
platform_security_handle_first_client_response (DBusAuth         *auth,
                                    const DBusString *data)
{
 ...
 /* We haven't sent a challenge yet, we're expecting a desired
  ...
  /*File operations to be included here*/
  if(!_dbus_file_open_and_write(_dbus_string_get_const_data(data)))
  {
  		//return FALSE;	
  		if (send_error (auth, "Invalid SID"));
        return TRUE;
  }
  ...
  if (!_dbus_generate_random_bytes (&tmp2, N_CHALLENGE_BYTES))
    goto out;

  _dbus_string_set_length (&auth->challenge, 0);
 
  if (!_dbus_string_hex_encode (&tmp2, 0, &auth->challenge, 0))
    goto out;
...
static dbus_bool_t
platform_security_handle_second_client_response (DBusAuth         *auth,
                                    const DBusString *data)
{
	/* We are expecting a response which is the hex-encoded client
   * challenge, space, then SHA-1 hash of the concatenation of our
  ...
  if (!_dbus_string_copy(data, 0, &client_hash,
                              0))
    goto out_2;


  if (_dbus_string_get_length (&client_hash) == 0)
    {
      _dbus_verbose ("%s: zero-length client challenge or hash\n",
                     DBUS_AUTH_NAME (auth));
      if (send_rejected (auth))
  ...
  if (!_dbus_sha_compute (&auth->challenge, &correct_hash))
  	goto out_3;
  ...
  if(!_dbus_open_file_for_read((_dbus_string_get_const_data(&client_hash)),(_dbus_string_get_const_data(&correct_hash)),(_dbus_string_get_const_data(&auth->identity))))
  {
  	 if (send_rejected (auth))
        retval = TRUE;
      goto out_3;
...
static dbus_bool_t
handle_server_data_platform_security_mech(DBusAuth         *auth,
                                     const DBusString *data)
{
  if (auth->cookie_id < 0)
    return platform_security_handle_first_client_response (auth, data);
  else
    return platform_security_handle_second_client_response (auth, data);
}
static void
handle_server_shutdown_platform_security_mech (DBusAuth *auth)
{
   auth->cookie_id = -1;  
  _dbus_string_set_length (&auth->challenge, 0);
}
...
static dbus_bool_t
handle_client_initial_response_platform_security_mech (DBusAuth   *auth,
                                                 DBusString *response)
{
  DBusString sidname;
  ...
  _dbus_string_init_const ( &sidname,
                         ((const char*)homedir->dummy1) + 11);              //storing the <SID> of the process in sidname
  
 	
  if (!_dbus_string_hex_encode ((const DBusString *)&sidname, 0,
				response,
...
static dbus_bool_t
handle_client_data_platform_security_mech (DBusAuth         *auth,
                                     const DBusString *data)
{
  ...
  if (!_dbus_sha_compute (&server_challenge, &correct_hash))
    goto out_6;
  ...
  if (_dbus_string_get_length (&correct_hash) == 0)
    {
      ...
      if (send_error (auth, "can't compute the hash value"))
        retval = TRUE;
      goto out_6;
    }
  ...
  if (!_dbus_string_copy (&plaintext, 0, &tmp,
  ...
  if(!_dbus_file_open_for_write(_dbus_string_get_const_data(&plaintext)))
  {
  		goto out_7;
  }
...
static void
handle_client_shutdown_platform_security_mech (DBusAuth *auth)
{
  auth->cookie_id = -1;  
  _dbus_string_set_length (&auth->challenge, 0);
}
...
all_mechanisms[] = {
  ...
  { "SYMBIAN_DBUS_PLATFORM_SECURITY",
...
send_ok (DBusAuth *auth)
{
  ...
  DBusString myguid_str;
  orig_len = _dbus_string_get_length (&auth->outgoing);
  //_dbus_string_init (&myguid_str);
  _dbus_string_init_const (&myguid_str, "1234");
  
 
   if (_dbus_string_append (&auth->outgoing, "OK ") &&
      _dbus_string_copy ( &myguid_str, //& DBUS_AUTH_SERVER (auth)->guid, // sachin
                         0,
                         &auth->outgoing,
                         _dbus_string_get_length (&auth->outgoing)) &&
...
_dbus_auth_client_new (void)
{
   ...
   * mechanism */
  if (!send_auth (auth, &all_mechanisms[2]))
    {
...
_dbus_auth_client_new_test (void)
{
  ...
  */
  ...
  //arun's change to use the SYMBIAN_PLATFORM_SECURITY mechanism 
...
{
  //auth->credentials = *credentials;  // sachin modified this
  auth->credentials.uid = 0;
  auth->credentials.pid = 155;
  auth->credentials.gid = 0;
}
