...
{
#ifdef __SYMBIAN32__
// Cant use reserved word new in Symbian gcce
  DBusKey *new_key;
#else
  DBusKey *new;
#endif
...
#ifdef __SYMBIAN32__
  new_key = (DBusKey *) dbus_realloc (keys, sizeof (DBusKey) * (n_keys + 1));
  if (new_key == NULL)
#else
  ...
  while (_dbus_string_pop_line (&contents, &line))
    {
      ...
      int end;
#ifdef __SYMBIAN32__
// Cannot use reserved word new in Symbian gcce
     DBusKey *new_key;
#else
      DBusKey *new;
#endif
...
#ifdef DBUS_BUILD_TESTS
#if EMULATOR
GET_STATIC_VAR_FROM_TLS(already_warned,_dbus_keyring_new_homedir,dbus_bool_t )
#define already_warned (*GET_DBUS_WSD_VAR_NAME(already_warned,_dbus_keyring_new_homedir,s)())
#endif // EMULATOR
