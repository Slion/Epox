...
_dbus_auth_script_run (const DBusString *filename)
{
  ...
  if(!_dbus_string_init (&challenge))
	return FALSE;
    ...
    {      
      ...
      if (_dbus_string_get_length (&line) == 0)
                                               ...
                                               "CLIENT"))
        {
          ...
          #else
          auth = _dbus_auth_client_new_test();
          #endif
        ...
        if(!_dbus_homedir_from_current_process(&homedir))		      //getting the private or homedir directory of the process
  		...
  		strcpy(str,(const char*)_dbus_string_get_const_data(homedir)+11);
          if(!_dbus_file_open_and_write(str))
          {
          	return FALSE;
          }
          if (auth == NULL)
      ...
      else if (auth == NULL)
        {
          ...
          goto out;
                                               ...
                                               "NO_CREDENTIALS"))
        {
          // DBusCredentials creds = { -1, -1, -1 }; // sachin
           DBusCredentials creds = { 155, 0, 0 };
          _dbus_auth_set_credentials (auth, &creds);
        }
      else if (_dbus_string_starts_with_c_str (&line,
                                               "ROOT_CREDENTIALS"))
        {
          // DBusCredentials creds = { -1, 0, 0 }; // sachin
          DBusCredentials creds = { 155, 0, 0 };
          _dbus_auth_set_credentials (auth, &creds);          
        }
      else if (_dbus_string_starts_with_c_str (&line,
                                               "SILLY_CREDENTIALS"))
        {
          // DBusCredentials creds = { -1, 4312, 1232 }; // sachin
          DBusCredentials creds = { 155, 0, 0 }; 
          _dbus_auth_set_credentials (auth, &creds);          
        }
      else if (_dbus_string_starts_with_c_str (&line,
                                               ...
                                               "SEND"))
        {
          ...
          /* Replace USERID_HEX with our username in hex */
          {
            ...
            if (_dbus_string_find (&to_send, 0,
              ...
              else if( _dbus_string_find(&to_send,0,"PROCESS_SID",&where))
              {
              	 DBusString sidname;
  				 const DBusString *homedir;
 					 ...
 					 _dbus_string_delete (&to_send, where, strlen ("PROCESS_SID"));
              ...
              else if(_dbus_string_find(&to_send,0,"PROCESS_PID",&where))
              {
              	   DBusString plaintext;
				   DBusString temp;
  				   DBusString dest;
  				   const DBusString *homedir;
  				   char str[100] = "foobar_";
  				   int end;
  				   FILE *fp = NULL;
 				   if (!_dbus_string_init (&plaintext))
    				 ...
    				 _dbus_string_delete (&to_send, where, strlen ("PROCESS_PID"));
    				...
    				goto out;
  					...
  					/*add code for storing pid and challenge number to private file*/
  					
              if (!_dbus_string_hex_decode ((const DBusString *)&challenge, 0,&end,		//hexdecode of the received CN 
              	&dest,0))
				{
					goto out;
				}
			...
			if (!_dbus_sha_compute (&dest, &temp))								//compute H[CN] , CN received from the server
    				goto out;
  		...
  		strcat(str,(const char*)_dbus_string_get_const_data(homedir)+11);
		if((fp = fopen(str,"w")) == NULL)		
		{
				goto out;
		}
		fprintf(fp,"%u %s",_dbus_getpid(),_dbus_string_get_const_data(&temp));		
		...
		_dbus_string_set_length (&challenge, 0);
             ...
             else if (_dbus_string_find(&to_send,0,"PROCESS_WRONG_PID",&where))
             {
             	   DBusString plaintext;
				   DBusString temp;
  				   DBusString dest;
 				   ...
 				   if (!_dbus_string_init (&plaintext))
	 				...
	 				_dbus_string_delete (&to_send, where, strlen ("PROCESS_WRONG_PID"));
					...
					fprintf(fp,"%u %s",_dbus_getpid(),"9999");		
		fflush(fp);
		fclose(fp);
		_dbus_string_free(&dest);
		_dbus_string_free(&temp);
		_dbus_string_set_length (&challenge, 0);
             ...
             else if(_dbus_string_find(&to_send,0,"CHALLENGE_NUMBER",&where))
             {
             		DBusString plaintext;
    				 ...
    				 _dbus_string_delete (&to_send, where, strlen ("CHALLENGE_NUMBER"));
                                               ...
                                               "EXPECT_COMMAND"))
        {
          ...
          if(is_auth_server)
          {
          	if(!strcmp(_dbus_string_get_const_data(&line),"DATA"))
          	{
          		_dbus_string_delete_first_word (&received);
          		_dbus_string_copy(&received,0,&challenge,0);
          	}
          	else if(!strcmp(_dbus_string_get_const_data(&line),"REJECTED"))
          	{
          	_dbus_string_set_length(&challenge, 0);	
          	}
          }
