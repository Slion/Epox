...
#define LOG_FILE "c:\\logs\\dbus_bus_get_private0_log1.txt"
...
void create_xml(int result)
	...
	testResultXml("dbus_bus_get_private0");
    close_log_file();
	}
...
int main()
{
	...
	connection1 = dbus_bus_get_private (DBUS_BUS_SESSION, &error);
	if(!connection1 || dbus_error_is_set(&error))
		return handle_error(&error);
	
	if( TestConnectionEqual(connection, connection1) )
	{
		std_log(LOG_FILENAME_LINE, "Different connections not created");
		create_xml(1);
		return 1;
	}
