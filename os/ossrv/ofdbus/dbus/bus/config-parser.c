                       ...
                       const BusConfigParser *parent)
{
  ...
  if (parent != NULL)
  ...
  else
    {
       ...
       */
#ifdef __SYMBIAN32__
      parser->limits.activation_timeout = 250000; /*  changed to 250 seconds */
#else
       ...
       */
      parser->limits.auth_timeout = 300000; /* 30 seconds bsr changed to 300 sec*/
      ...
      /*
      parser->limits.max_incoming_bytes = 1024 * 1024 * 63;
      parser->limits.max_outgoing_bytes = 1024 * 1024 * 63;
      parser->limits.max_message_size = 1024 * 1024 * 32;
      ...
      parser->limits.max_services_per_connection = 256;
      parser->limits.max_match_rules_per_connection = 512;
      parser->limits.reply_timeout = 5 * 60 * 1000;
      parser->limits.max_replies_per_connection = 32;
      */
...
#else    
  // always give absolute path in config files on symbian
     return _dbus_string_copy (filename, 0, full_path, 0);
#endif
}
...
#else
DBUS_DATADIR"\\dbus1\\services",
/*   Certain files such as jabber.service are exported and included in the rom builds by other components. 
 *   These are not available for emulator environments and hence added explictly for hardware platforms.
 */
#if (! defined __WINSCW__)
"z:\\data\\dbus\\dbus1\\services",
#endif
...
#ifdef __SYMBIAN32__
      if (!_dbus_string_ends_with_c_str (&path, "\\dbus\\dbus1\\services"))
#else
  ...
  while ((link = _dbus_list_pop_first_link (&dirs)))
    {
      ...
      if (strcasecmp (test_service_dir_matches[i], 
                  (char *)link->data) != 0)
        {
